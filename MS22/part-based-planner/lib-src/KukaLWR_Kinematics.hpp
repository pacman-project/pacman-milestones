...
public:
    ...
    static bool ikSolverAnalytical7DOF( Eigen::Affine3d & poseDsr, std::vector<double> & jntPosDsr);
    //! Inverse kinematics of the KUKA LWR Robot (6 dof). Returns 1 if the requested position is reachable
    static bool ikSolver(std::vector<double> & jntPosMsr, Eigen::Affine3d & poseDsr, std::vector<double> & jntPosDsr);
	//! Inverse kinematics of the KUKA LWR Robot (7 dof). Returns 1 if the requested position is reachable
	static bool ikSolverIterative7DOF(std::vector<double> & jntPosMsr, Eigen::Affine3d & poseDsr, std::vector<double> & jntPosDsr);
    //! Forward kinematics of the KUKA LWR Robot (7 dof). Returns 1 if the requested position is reachable
	static bool fkSolver(const std::vector<double> & jntPosDsr, Eigen::Affine3d & poseDsr);
