...
bool KukaLWR_Kinematics::ikSolver(std::vector<double> & jntPosMsr, Eigen::Affine3d & poseDsr, std::vector<double> & jntPosDsr)
{
	// Returning value
	bool output = true;
	...
	Eigen::Quaterniond poseDsr_rot(poseDsr.rotation());
	Eigen::Translation<double,3> poseDsr_trans(poseDsr.translation());
  
	KDL::Rotation R = KDL::Rotation::Quaternion(poseDsr_rot.x(),poseDsr_rot.y(),poseDsr_rot.z(),poseDsr_rot.w());
	KDL::Vector p (poseDsr_trans.x(), poseDsr_trans.y(), poseDsr_trans.z());
...
bool KukaLWR_Kinematics::ikSolverIterative7DOF(std::vector<double> & jntPosMsr, Eigen::Affine3d & poseDsr, std::vector<double> & jntPosDsr) {
	// Define chain
	...
	KDL::Rotation RTest = KDL::Rotation::Quaternion(poseDsr_rot.x(),poseDsr_rot.y(),poseDsr_rot.z(),poseDsr_rot.w());
	KDL::Vector pTest(poseDsr_trans.x(), poseDsr_trans.y(), poseDsr_trans.z());
	
	
	KDL::Frame F_dest(RTest, pTest);
...
bool KukaLWR_Kinematics::fkSolver(const std::vector<double> & jntPosDsr, Eigen::Affine3d & poseDsr)
{
...
bool KukaLWR_Kinematics::ikSolverAnalytical7DOF( Eigen::Affine3d & poseDsr, std::vector<double> & jntPosDsr)
{
	...
	#if DEBUG_IK
	cout << "IK begins!" << endl;	
        cout << " x= "<<poseDsr_trans.x()<<" y= "<<poseDsr_trans.y()<<" z= "<< poseDsr_trans.z() << endl;
	#endif
	...
	KDL::Rotation R70des = KDL::Rotation::Quaternion(poseDsr_rot.x(),poseDsr_rot.y(),poseDsr_rot.z(),poseDsr_rot.w());
	KDL::Vector x70des(poseDsr_trans.x(), poseDsr_trans.y(), poseDsr_trans.z());
	...
	#if DEBUG_IK 
	cout << "[Debug:]Segfault Line: 519 "<< endl << flush;
	#endif
	...
	do{
	#if DEBUG_IK 
	cout << "[Debug:]Segfault Line: 541 " << i << endl << flush;
	#endif
		for (j=0;j< array2D.size();j++)
		{
		  cout << "[Debug:]Segfault Line: 530 " << i << j << endl << flush;
			for (k=0; k< (unsigned int)set_number[i+1]; k++)
			{
			  cout << "[Debug:]Segfault Line: 533 " << i << j << k << endl << flush;
			  
			  //		
			  cout << "array3dsizes" << array3D.at(0).size() << endl << flush;
			  cout << "array3dsizes" << array3D.at(1).size() << endl << flush;
			  cout << "array3dsizes" << array3D.at(2).size() << endl << flush;
			  cout << "array3dsizes" << array3D.at(3).size() << endl << flush;
			  cout << "array3dsizes" << array3D.at(4).size() << endl << flush;
			  cout << "array3dsizes" << array3D.at(5).size() << endl << flush;
			  cout << "array3dsizes" << array3D.at(6).size() << endl << flush;
			  
			  
			  cout << "array3dsize" << array3D.size() << endl << flush;
			  cout << "array3dsize" << array3D.at(i+1).size() << endl << flush;
			  cout << "arraysize" << array3D.at(i+1).at(k).size() << endl << flush;
			  cout << "[Debug:]Segfault Line: 538new(after array3d) " << i << j << k << endl << flush;
				psiset = set_intersection (array2D[j], array3D[i+1][k], intersection_is_empty );
				//
				cout << "[Debug:]Segfault Line: 536 " << i << j << k << endl << flush;
				if (intersection_is_empty == false)
				{
				  cout << "[Debug:]Segfault Line: 538 " << i << j << k << endl << flush;
...
std::vector<double> set_intersection (std::vector<double> set1, std::vector<double> set2, bool &intersection_is_empty )
{
  cout << "[Debug:]Segfault Line: 1249 (in set_intersection) " << endl << flush;
